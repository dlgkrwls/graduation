...
#        self.smooth_model_3d = util.setup_smooth_model(self.checkpoint_path_3D)
 ...
 #       smooth_coords_3d = util.apply_3Dsmoothing(coords_3d_data,self.smooth_model_3d, False,'smoothed_3D_pose_data.npy')
        ...
        output_file = "process_data/pose_3d_test.mp4"
        ...
        with writer.saving(fig, output_file, dpi=100):
            for frame_idx in range(len(front_smoothed_data)):
                ...
                x_vals = [point[0] for point in smooth_coords_3d]
                y_vals = [point[1] for point in smooth_coords_3d]
                z_vals = [point[2] for point in smooth_coords_3d]

                # 3D 랜드마크 플로팅
                ax.scatter(x_vals, z_vals, -np.array(y_vals), c='r', marker='o')
                for i in range(len(SKELETON)):
                    kpt_a, kpt_b = SKELETON[i][0], SKELETON[i][1]
                    x_a, y_a, z_a = x_vals[kpt_a],-y_vals[kpt_a],z_vals[kpt_a]
                    x_b, y_b, z_b = x_vals[kpt_b],-y_vals[kpt_b],z_vals[kpt_b]
...
#     output_front_file = 'process_data/smooth_detect_5_squart_front_class.mp4'
#     output_side_file = 'process_data/smooth_detect_5_squart_class.mp4'
