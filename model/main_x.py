...
class Gp_main:
    ...
    # 왜곡 보정 이미지 반환
    def undistort_image(self,frame, camera_matrix, dist_coeffs):
    ...
    # 스케일링된 3D 좌표 반환
    def scale_3d_coords(self,coords_3d):
        ...
        for part, coord in coords_3d.items():
            scaled_x = self.scale_value(coord[0][0], x_min, x_max)
            scaled_y = self.scale_value(coord[1][0], y_min, y_max)
            scaled_z = self.scale_value(coord[2][0], z_min, z_max)
    ...
    def extract_camera_coords(self,landmarks, frame):
        camera_coords = {
                    "nose": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.NOSE.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.NOSE.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "left_shoulder": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.LEFT_SHOULDER.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.LEFT_SHOULDER.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "right_shoulder": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "left_elbow": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.LEFT_ELBOW.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.LEFT_ELBOW.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "right_elbow": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.RIGHT_ELBOW.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.RIGHT_ELBOW.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "left_wrist": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.LEFT_WRIST.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.LEFT_WRIST.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "right_wrist": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.RIGHT_WRIST.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.RIGHT_WRIST.value].y * frame.shape[0]],
                        dtype=np.float32),
                    ...
                    "right_hip": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.RIGHT_HIP.value].x * frame.shape[1],
                    ...
                    "right_knee": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.RIGHT_KNEE.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.RIGHT_KNEE.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "left_ankle": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.LEFT_ANKLE.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.LEFT_ANKLE.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "right_ankle": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.RIGHT_ANKLE.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.RIGHT_ANKLE.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "left_heel": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.LEFT_HEEL.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.LEFT_HEEL.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "right_heel": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.RIGHT_HEEL.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.RIGHT_HEEL.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "left_foot": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.LEFT_FOOT_INDEX.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.LEFT_FOOT_INDEX.value].y * frame.shape[0]],
                        dtype=np.float32),

                    "right_foot": np.array(
                        [landmarks[self.mp_pose.PoseLandmark.RIGHT_FOOT_INDEX.value].x * frame.shape[1],
                        landmarks[self.mp_pose.PoseLandmark.RIGHT_FOOT_INDEX.value].y * frame.shape[0]],
                        dtype=np.float32),
    ...
    def count_injury(self,stance,knee_angle,knee_position,count_list,health_warning):
    ...
    def main(self,path1=None,path2=None):
        camera_matrix1, dist_coeffs1, camera_matrix2, dist_coeffs2 = self.setup_camera()
        ...
        if path1 is None or path2 is None:
            print("Didn't have video pathes")
            return 0
            ...
            # pose model 선언#
            pose_model = self.setup_pose_model()
            ...
            while True:
                ...
                # 왜곡 보정된 이미지
                frame1_undistorted = self.undistort_image(frame1, camera_matrix1, dist_coeffs1)
                frame2_undistorted = self.undistort_image(frame2, camera_matrix2, dist_coeffs2)
                ...
                if pose_results1.pose_landmarks and pose_results2.pose_landmarks:
                        # 각 관절 좌표
                        front_coords1 = self.extract_camera_coords(pose_results1.pose_landmarks.landmark, frame1_undistorted)
                        side_coords2 = self.extract_camera_coords(pose_results2.pose_landmarks.landmark, frame2_undistorted)
                        ...
                        # 2D 좌표 찍기
                        img1_landmarks = self.draw_2d_landmarks(frame1_undistorted,front_coords1,connections,body_parts)
                        img2_landmarks =  self.draw_2d_landmarks(frame2_undistorted,side_coords2,connections_right,body_parts_right)
                else:
                    print(frame_idx)
                ...
                if front_coords1 is not None and side_coords2 is not None:
                    # 이전 2D 좌표 사용해서 그리기
                    img1_landmarks =  self.draw_2d_landmarks(frame1_undistorted, front_coords1, connections, body_parts)
                    img2_landmarks =  self.draw_2d_landmarks(frame2_undistorted, side_coords2, connections_right,
                                                    body_parts_right)
            ...
            for i in range(len(count_list)):
                print( self.get_time_in_seconds(count_list[i],fps1))
            # 스쿼트 횟수별 부상여부 감지
            self.count_injury(stance_list, knee_position_list, knee_angle_list, count_list, health_warning)

            with open("data/test.json", 'w') as f:  #(Modify)json download path
